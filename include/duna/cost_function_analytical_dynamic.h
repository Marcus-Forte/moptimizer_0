...
namespace duna
{
    /* Analytical cost function module. Computes hessian using provided `f_df` model function with explicit jacobian calculation. Dynamic size parameters and function outputs. */
    ...
    public:
        ...
        CostFunctionAnalyticalDynamic(ModelPtr model, int num_parameters, int num_outputs, int num_residuals) : CostFunctionAnalytical<Scalar>(model, num_residuals), 
        num_parameters_(num_parameters), num_outputs_(num_outputs)
        {}

        CostFunctionAnalyticalDynamic(ModelPtr model, int num_parameters, int num_outputs ) : CostFunctionAnalytical<Scalar>(model), 
        num_parameters_(num_parameters), num_outputs_(num_outputs)
        {}
