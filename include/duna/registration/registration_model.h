...
namespace duna
{
    ...
    struct Point2Plane
    {
        ...
        Point2Plane(const pcl::PointCloud<PointSource> &source_, const pcl::PointCloud<PointSource> &target_, const pcl::PointCloud<pcl::Normal>& normals_, const pcl::Correspondences &corrs_) : source(source_),
                                                                                                                                                                                              target(target_),
                                                                                                                                                                                              target_normals(normals_),
                                                                                                                                                                                              corrs(corrs_)
        {
        }
        void setup(const Scalar *x)
        ...
        void operator()(const Scalar *x, Scalar *f_x, unsigned int index)
        {
            const PointSource &src_pt = source.points[corrs[index].index_query];
            const PointSource &tgt_pt = target.points[corrs[index].index_match];
            const pcl::Normal &tgt_normal = target_normals.points[corrs[index].index_match];


            Eigen::Matrix<Scalar, 4, 1> src_(src_pt.x, src_pt.y, src_pt.z, 1);
            Eigen::Matrix<Scalar, 4, 1> tgt_(tgt_pt.x, tgt_pt.y, tgt_pt.z, 0);
            Eigen::Matrix<Scalar, 4, 1> tgt_normal_(tgt_normal.normal_x, tgt_normal.normal_x, tgt_normal.normal_x, 0);
