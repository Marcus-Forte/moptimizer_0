...
namespace duna
{
    ...
    public:
        ...
        virtual bool f_df(const Scalar *x, Scalar *f_x, Scalar *jacobian, unsigned int index) override
        {
            ...
            // Eigen::Matrix<Scalar, 3, 3> source_skew;

            // source_skew << SKEW_SYMMETRIC_FROM(src_);

            // Eigen::Matrix<Scalar, 3, 3> jacobian__;
            ...
            // so3::Exp<Scalar>(x_map, exp);

            // jacobian__ = -exp * source_skew * right_jacobian;

            // Eigen::Matrix<Scalar, 3, 3> hessian_;

            // hessian_ = jacobian__.transpose() * jacobian__;
