...
TYPED_TEST(RegistrationPoint2Plane, Translation) {
  ...
  scan_matcher_model.reset(new duna::ScanMatching6DOFPoint2Plane<PointT, PointT, TypeParam>(
      this->source, this->target, this->target_kdtree));
  auto cost =
      new duna::CostFunctionNumerical<TypeParam, 6, 1>(scan_matcher_model, this->source->size());
