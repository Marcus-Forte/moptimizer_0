...
TYPED_TEST(RegistrationPoint2Point, Translation) {
  ...
  pcl::transformPointCloud(*this->target, *this->source,
                           this->reference_transform);

  typename duna::ScanMatching6DOFPoint2Point<PointT, PointT, TypeParam>::Ptr
      scan_matcher_model;
  scan_matcher_model.reset(
      new duna::ScanMatching6DOFPoint2Point<PointT, PointT, TypeParam>(
          this->source, this->target, this->target_kdtree));

  auto cost = new duna::CostFunctionNumerical<TypeParam, 6, 3>(
      scan_matcher_model, this->source->size());
  ...
  std::cout << "Final X " << Eigen::Map<Eigen::Matrix<TypeParam, 6, 1>>(x0)
            << std::endl;
  std::cout << "Final Transform: " << this->result_transform << std::endl;
  std::cout << "Reference Transform: " << reference_transform_inverse
            << std::endl;

  for (int i = 0; i < reference_transform_inverse.size(); ++i) {
    EXPECT_NEAR(this->result_transform(i), reference_transform_inverse(i),
                TOLERANCE);
  }
...
TYPED_TEST(RegistrationPoint2Point, RotationPlusTranslation) {
  ...
  rot =
      Eigen::AngleAxis<TypeParam>(0.3,
                                  Eigen::Matrix<TypeParam, 3, 1>::UnitX()) *
      Eigen::AngleAxis<TypeParam>(0.4,
                                  Eigen::Matrix<TypeParam, 3, 1>::UnitY()) *
...
TYPED_TEST(RegistrationPoint2Point, RotationPlusTranslationDynamic) {
  ...
  auto cost = new duna::CostFunctionAnalyticalDynamic<TypeParam>(
      scan_matcher_model, 6, 3, this->source->size());
