...
template <typename Scalar>
inline void convert3DOFParameterToMatrix(
    const Scalar *x, Eigen::Matrix<Scalar, 4, 4> &transform_matrix_) {
  transform_matrix_.setIdentity();
  transform_matrix_(3, 3) = 1;
...
template <typename Scalar>
inline void convert3DOFParameterToMatrix3(
...
inline void Log(const Eigen::Ref<Eigen::Matrix<Scalar, 3, 3>> &R,
  ...
  Scalar theta =
      (R.trace() > 3.0 - 1e-6) ? 0.0 : std::acos(0.5 * (R.trace() - 1));
  Eigen::Matrix<Scalar, 3, 1> K(R(2, 1) - R(1, 2), R(0, 2) - R(2, 0),
                                R(1, 0) - R(0, 1));
...
void inverseRightJacobian(
  ...
  Scalar factor = 1 / r.squaredNorm() -
                  (1 + cos(r.norm())) / (2 * r.norm() * sin(r.norm()));
  inv_jacobian = Eigen::Matrix<Scalar, 3, 3>::Identity() + 0.5 * r_skew +
                 factor * r_skew * r_skew;
}
...
template void DUNA_OPTIMIZER_EXPORT convert6DOFParameterToMatrix<float>(
    const float *x, Eigen::Matrix<float, 4, 4> &transform_matrix_);

template void DUNA_OPTIMIZER_EXPORT convert3DOFParameterToMatrix<double>(
    const double *x, Eigen::Matrix<double, 4, 4> &transform_matrix_);
template void DUNA_OPTIMIZER_EXPORT convert3DOFParameterToMatrix<float>(
    const float *x, Eigen::Matrix<float, 4, 4> &transform_matrix_);

template void DUNA_OPTIMIZER_EXPORT convert3DOFParameterToMatrix3<float>(
    const float *x, Eigen::Matrix<float, 3, 3> &transform_matrix_);
...
template void DUNA_OPTIMIZER_EXPORT
Log<double>(const Eigen::Ref<Eigen::Matrix<double, 3, 3>> &R,
            Eigen::Matrix<double, 3, 1> &delta);
template void DUNA_OPTIMIZER_EXPORT
Log<float>(const Eigen::Ref<Eigen::Matrix<float, 3, 3>> &R,
           Eigen::Matrix<float, 3, 1> &delta);

template void DUNA_OPTIMIZER_EXPORT inverseRightJacobian<float>(
    const Eigen::Matrix<float, 3, 1> &r,
    Eigen::Ref<Eigen::Matrix<float, 3, 3>> inv_jacobian);
template void DUNA_OPTIMIZER_EXPORT inverseRightJacobian<double>(
    const Eigen::Matrix<double, 3, 1> &r,
    Eigen::Ref<Eigen::Matrix<double, 3, 3>> inv_jacobian);
